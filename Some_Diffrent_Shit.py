...
def assign_images(decibel_levels, avg_decibel, max_decibel, window_size=40):
    ...
    for level in smoothed_levels:
        ...
        if level == 0:
            images.append('./gwack/line.png')
        elif 0 <= level < avg_decibel * 3 / 4:
            choice = random.choice(['./gwack/Teeth_ee.png', './gwack/Teeth.png', './gwack/circle.png', './gwack/Tounge.png'])
...
def main(path):
    ...
    create_video(assigned_images, video_output_path_avi, codec='DIVX', fps=24)
    create_video(assigned_images, video_output_path_mp4, codec='mp4v', fps=24)
